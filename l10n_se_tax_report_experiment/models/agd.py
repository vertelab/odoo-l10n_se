...
class account_agd_declaration(models.Model):
    ...
    def do_draft(self):
        for record in self:
            super(account_agd_declaration, record).do_draft()
            record.slip_ids = []
    ...
    def calculate(self): # make a short cut to print financial report
        for record in self:
            ...
            slips = record.env['hr.payslip'].search([('move_id.period_id.id','=',record.period_start.id)])
            record.payslip_ids = slips.mapped('id')
            record.move_ids = []
            for move in slips.mapped('move_id'):
                move.agd_declaration_id = record.id
            ...
            ctx = {
                ...
                'accounting_method': record.accounting_method,
                'target_move': record.target_move,
                'nix_journal_ids': [record.env.ref('l10n_se_tax_report.agd_journal').id]
            }
            ...
            for row in TAGS:
                line = record.env.ref('l10n_se_tax_report.agd_report_%s' % row)
                if not line:
                    raise Warning(_('Report line missing %' % row))
                record.env['account.declaration.line'].create({
                    'agd_declaration_id': record.id,
            ...
            tax_account = record.env['account.tax'].search([('tax_group_id', '=', record.env.ref('l10n_se.tax_group_hr').id), ('name', 'not in', ['eSKDUpload', 'Ag', 'AgBrutU', 'AgAvgU', 'AgAvgAv', 'AgAvg', 'AgAvd', 'AgAvdU', 'AgAvgPreS', 'AgPre', 'UlagVXLon', 'AvgVXLon'])])
            def parse_xml(recordsets):
                def get_tax_value(tax):
                    line = record.env.ref('l10n_se_tax_report.agd_report_%s' % tax)
                ...
                company_registry = '16' + record.env.user.company_id.company_registry.replace('-', '')
                ...
                avsandare = record.env['ir.config_parameter'].get_param('l10n_se_tax_report.agd_avsandare')
                if not avsandare:
                ...
                arbetsgivarekontaktperson = record.env['ir.config_parameter'].get_param('l10n_se_tax_report.ag_contact')
                if not arbetsgivarekontaktperson:
                    raise Warning(_(u'Please configurate arbetsgivare kontaktperson'))
                for ak in serecordlf.env['res.partner'].browse(eval(arbetsgivarekontaktperson)):
                ...
                # Uppgift 1 HU
                period = record.period_start.date_start[:4] + record.period_start.date_start[5:7]
                ...
                # Uppgift IU
                seq = 1
                for slip in record.payslip_ids:
            ...
            def schema_validate(xml):
                ...
                try:
                    ...
                    record.message_post(body=e.message)
                    return False
                return True
            ...
            tax_accounts = record.env['account.tax'].search([('name', 'in', ['SkAvdrLon', 'AvgHel'])])
            kontoskatte = tax_accounts.mapped('account_id')
            _logger.warning('kontoskatte %s' % kontoskatte)
            
            agd_journal_id = record.env['ir.config_parameter'].get_param('l10n_se_tax_report.agd_journal')
            if not agd_journal_id:
                raise Warning('Konfigurera din arbetsgivardeklaration journal!')
            else:
                agd_journal = record.env['account.journal'].browse(int(agd_journal_id))

...
class account_periodic_compilation(models.Model):
    ...
    def do_draft(self):
        for record in self:
            for invoice in record.env['account.invoice'].search([( 'periodic_compilation_id', '=', record.id  )]):
    ...
    def calculate(self): # make a short cut to print financial report
        for record in self:
            ...
            for invoice in record.env['account.invoice'].search([('period_id.id', '=', record.period_start.id)]):
                ...
                if (pc_supplied_goods + pc_triangulation + pc_services_supplied) > 0:         
                    invoice.periodic_compilation_id = record.id
                
                    if invoice.partner_id.id in partner_ids:
                        # ~ _logger.warn("\n\n22222 hellooo :: ")
                        line = record.line_ids.filtered( lambda l: l.partner_id == invoice.partner_id )
                        ...
                        record.env['account.declaration.line'].create({
                            'periodic_compilation_id': record.id,

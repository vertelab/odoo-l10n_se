...
def mis_xml(r_lst,b_lst):
    ...
    def create_recs(data,rec,report_id,seq):
        def kpi(data,report_id,rec,report_style,seq):
            ...
            if rec.get('account_ids'):
                ...
                kpiexp = ET.SubElement(data, 'record', id='kpi_title_%s_%s' % (report_id,rec['element_name']), model="mis.report.kpi.expression")
                ET.SubElement(kpiexp, "field", name="kpi_id", ref='%s_%s' % (report_id,rec['element_name']))
                ET.SubElement(kpiexp, "field", name="name").text = 'bal%s%s' % ([int(a) for a in rec['account_ids']],'* -1' if rec['sign'] == '-1' else '')
    ...
    def create_title(data,rec,report_id,account_ids,report_style,seq):
        def kpi(data,report_id,rec,account_ids,report_style,seq):
            kpi = ET.SubElement(data, 'record', id='%s_%s_title' % (report_id,rec['element_name']), model="mis.report.kpi")
            ET.SubElement(kpi, "field", name="report_id",ref="%s" % report_id)
            ET.SubElement(kpi, "field", name="name").text = rec['element_name']
            ET.SubElement(kpi, "field", name="description").text = rec['name']
            ET.SubElement(kpi, "field", name="style_id", ref=report_style)
    ...
    for seq,rec in enumerate(r_lst,start=1):
        ...
        if rec['element_name'] =='RorelseresultatAbstract':
            ...
            create_title(data,rec,'report_rr','Nettoomsattning+ForandringLagerProdukterIArbeteF+AktiveratArbeteEgenRakning+OvrigaRorelseintakter','report_style_5','%s' % seq)
...
# ~ 8 Rörelseintäkter, lagerförändringar m.m. RorelseintakterLagerforandringar title
            ...
            create_title(data,rec,'report_rr','HandelsvarorKostnader+OvrigaExternaKostnader+Personalkostnader+AvskrivningarNedskrivningarMater+NedskrivningarOmsattningstillgan+OvrigaRorelsekostnader','report_style_4','%s' % seq)
...
# ~ 17 Rörelsekostnader Rorelsekostnader title
            ...
            create_title(data,rec,'report_rr','RorelseintakterLagerforandringar-Rorelsekostnader','report_style_5','%s' % seq)
# ~ 18 Rörelseresultat Rorelseresultat title

        elif rec['element_name'] =='FinansiellaPoster':
            create_title(data,rec,'report_rr','ResultatAndelarKoncernforetag+ResultatAndelarIntresseforetagGe+ResultatOvrigaforetagAgarintress+ResultatOvrigaFinansiellaAnlaggn+OvrigaRanteintakterLiknandeResul+NedskrivningarFinansiellaAnlaggn+RantekostnaderLiknandeResultatpo','report_style_5','%s' % seq)
...
# ~ 28 Resultat efter finansiella poster ResultatEfterFinansiellaPoster title
            ...
            create_title(data,rec,'report_rr','ErhallnaKoncernbidrag+LamnadeKoncernbidrag+ForandringPeriodiseringsfond+ForandringOveravskrivningar+OvrigaBokslutsdispositioner','report_style_5','%s' % seq)
...
# ~ 35 Summa bokslutsdispositioner Bokslutsdispositioner title
            ...
            create_title(data,rec,'report_rr','ResultatEfterFinansiellaPoster-Bokslutsdispositioner','report_style_5','%s' % seq)

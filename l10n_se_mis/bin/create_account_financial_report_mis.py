...
def mis_xml(r_lst, b_lst):
    def create_kpi(data, report_id, main_record, element_name, account_ids, sign, name, report_style, seq):
        ...
        if account_ids:
            kpiexp = ET.SubElement(data, 'record', id='mis_kpi_exp_%s' % element_name[:22],
                                   model="mis.report.kpi.expression")
            ET.SubElement(kpiexp, "field", name="kpi_id", ref='%s' % element_name[:22])
            ET.SubElement(kpiexp, "field", name="name").text = 'balp%s%s' % (
            [int(a) for a in account_ids], ' * -1' if sign == '-1' else '')
    ...
    def create_recs_long(data, record_id, main_record, name, desc, main_desc, lst, seq):
        ...
        kpiexp = ET.SubElement(data, 'record', id='mis_main_kpi_exp_%s_%s' % (name, name),
                               model="mis.report.kpi.expression")
        ...
        for rec in lst:
            if rec.get('parent_id') == record_id and rec.get('account_ids'):
                create_kpi(data, record_id, rec['element_name'], rec['element_name'], rec.get('account_ids'),
                           rec.get('sign'), rec['name'], 'report_style_2', seq)
                create_kpi(data, record_id, '%s_%s' % (record_id, rec['element_name']), rec['element_name'],
                           rec.get('account_ids'), rec.get('sign'), rec['name'], 'report_style_2', seq)
                ...
                kpiexpmenu = ET.SubElement(data, 'record', id='mis_kpi_menu_%s' % rec['element_name'][:22],
                                           model="mis.report.kpi.expression")
    ...
    def create_recs(data, record_id, main_record, name, desc, main_desc, lst, seq):
        ...
        def kpi(data, report_id, element_name, account_ids, name, report_style, seq):
            ...
            ET.SubElement(kpi, "field", name="sequence").text = seq
            if account_ids:
                kpiexp = ET.SubElement(data, 'record', id='mis_kpi_exp_%s_%s' % (report_id, element_name),
                                       model="mis.report.kpi.expression")
    ...
    def create_title(data, record_id, report_id, sum_accounts, desc, seq):
        def kpi(data, report_id, record_id, account_ids, desc, report_style, seq):
            ...
            if sum_accounts:
                ET.SubElement(kpi, "field", name="budgetable").text = 'True'
                kpiexp = ET.SubElement(data, 'record', id='kpi_title_%s_%s' % (report_id, record_id),
                                       model="mis.report.kpi.expression")
    ...
    create_recs(data, 'RorelsensIntakterLagerforandringarMmAbstract', 'report_rr', 'netto', u'Nettoomsättning',
                u'RR Intäkter', r_lst, '1')
    ...
    create_recs(data, 'ResultatrakningKostnadsslagsindeladAbstract', 'report_rr', 'kostslag', u'Kostnadsslag',
                u'RR Kostnadsslag', r_lst, '3')
    create_recs(data, 'RorelseresultatAbstract', 'report_rr', 'resultat', u'Resultat', u'RR Resultat', r_lst, '4')

    create_recs(data, 'FinansiellaPosterAbstract', 'report_rr', 'fin', u'Finansiella poster', u'RR Finansiella poster',
                r_lst, '5')
    create_recs(data, 'BokslutsdispositionerAbstract', 'report_rr', 'bokdisp', u'Bokslutsdispositioner',
                u'RR Bokslutsdispositioner', r_lst, '6')
    ...
    create_recs(data, 'MateriellaAnlaggningstillgangarAbstract', 'report_br', 'tillgang', u'Tillgångar',
                u'BR Tillgångar', b_lst, '2')
    create_recs(data, 'ObeskattadeReserverAbstract', 'report_br', 'obesk', u'Obeskattade reserver',
                u'BR Obeskattade reserver', b_lst, '3')
    create_recs(data, 'AvsattningarAbstract', 'report_br', 'avsatt', u'Avsättningar', u'BR Avsättningar', b_lst, '4')
    create_recs(data, 'FrittEgetKapitalAbstract', 'report_br', 'frittek', u'Fritt eget kapital',
                u'BR Fritt eget kapital', b_lst, '5')
    create_recs(data, 'VarulagerMmAbstract', 'report_br', 'lager', u'Varulager', u'BR Varulager', b_lst, '6')
    create_recs(data, 'KortfristigaFordringarAbstract', 'report_br', 'kortafordr', u'Kortfristiga fordringar',
                u'BR Kortfristiga fordringar', b_lst, '7')
    create_recs(data, 'ImmateriellaAnlaggningstillgangarAbstract', 'report_br', 'imtillg', u'Immatriella tillgångar',
                u'BR Immatriella tillgångar', b_lst, '8')
    create_recs(data, 'KassaBankAbstract', 'report_br', 'kassa', u'Kassa/Bank', u'BR Kassa/bank', b_lst, '9')
    create_recs(data, 'BundetEgetKapitalAbstract', 'report_br', 'bundek', u'Bundet eget kapital',
                u'BR Bundet eget kapital', b_lst, '10')
    create_recs(data, 'OmsattningstillgangarAbstract', 'report_br', 'omtillg', u'Omsättningstillgångar',
                u'BR Omsättningstillgångar', b_lst, '11')
    create_recs(data, 'KortfristigaSkulderAbstract', 'report_br', 'kortaskuld', u'Kortfristiga skulder',
                u'BR Kortfristiga skulder', b_lst, '12')
    create_recs(data, 'EgetKapitalSkulderAbstract', 'report_br', 'ekskuld', u'Eget kapital/skulder',
                u'BR Eget katpital/skulder', b_lst, '13')
    create_recs(data, 'FinansiellaAnlaggningstillgangarAbstract', 'report_br', 'fintillg',
                u'Finansiella anläggningstillgångar', u'BR Finansiella anläggningstillgångar', b_lst, '13')
    create_recs(data, 'EgetKapitalAbstract', 'report_br', 'ek', u'Eget kapital', u'BR Eget kapital', b_lst, '13')
    create_recs(data, 'AnlaggningstillgangarAbstract', 'report_br', 'anltillgabs', u'Antlägggningstillgångar abs',
                u'BR Anläggningstillgångar abs', b_lst, '13')
    create_recs(data, 'Anlaggningstillgangar', 'report_br', 'anltillg', u'Anläggningstillgångar',
                u'BR Anläggningstillgångar', b_lst, '13')
    create_recs(data, 'Omsattningstillgangar', 'report_br', 'omstillg', u'Omsättningstillgångar',
                u'BR Omsättningstillgångar', b_lst, '13')
    create_recs(data, 'TillgangarAbstract', 'report_br', 'tillg', u'Tillgångar', u'BR Tillgångar', b_lst, '13')
    create_recs(data, 'KortfristigaPlaceringarAbstract', 'report_br', 'kortpla', u'Kortfristiga placeringar',
                u'BR Kortfristiga placeringar', b_lst, '13')
    create_recs(data, 'BalansrakningAbstract', 'report_br', 'balans', u'Balansräkning', u'BR Balansräkning', b_lst,
                '13')
    create_recs(data, 'EgetKapitalSkulder', 'report_br', 'ekskulda', u'Egetkapital/skulder', u'BR Eget kapital/skulder',
                b_lst, '13')

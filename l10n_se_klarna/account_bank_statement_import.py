...
from .klarna import KlarnaTransaktionsrapportType as Parser
import base64
import re
...
class AccountBankStatementImport(models.TransientModel):
    """Add Klarna method to account.bank.statement.import."""
    ...
    def _parse_file(self, data_file):
        ...
        try:
            _logger.info(u"Try parsing with Klarna Report file.")
            parser = Parser(base64.b64decode(self.data_file))
        except ValueError:
            _logger.info(u"Statement file was not a Klarna Report file.")
            return super(AccountBankStatementImport, self)._parse_file(data_file)

        fakt = re.compile('\d+')  # Pattern to find invoice numbers

        klarna = parser.parse()
        for s in klarna.statements:
        ...
        _logger.debug("res: %s" % klarna.statements)
        return klarna.account_currency, klarna.account_number, klarna.statements

...
class import_chart_of_account(models.TransientModel):
    ...
    _description = "Import Chart Template"
    ...
    def chart_title(self, code, name, last_account_class):
        ...
        if isinstance(code, basestring) and len(code) > 0 and ord(code[0]) in range(ord('0'), ord('9')):
            ...
            code = code if code.split()[0] in ['5-6', '30-34', '40-45', u'30\u201334', u'40\u2013',
                                               u'5\u20136'] else int(code.split()[0])
...
45 """]:  # kontogrupp
    ...
    def chart_normal(self, ws):
        ...
        for l in range(0, ws.nrows):
            ...
            (last_account_class, last_account_group, root_account) = self.chart_title(ws.cell_value(l, 2),
                                                                                      ws.cell_value(l, 3),
                                                                                      last_account_class)
            ...
            if ws.cell_value(l, 2) in range(1000, 9999):
                ...
                reconcile = ws.cell_value(l, 2) in range(1900, 2000) or ws.cell_value(l, 2) in [1630] or ws.cell_value(
                    l, 5) in range(1900, 2000) or ws.cell_value(l, 5) in [1630]
                ...
                if ws.cell_value(l, 5) in range(1000, 9999):
                    last_sub_account = self.create_account(ws.cell_value(l, 5), ws.cell_value(l, 6), last_account,
                                                           chart)
    ...
    def chart_simplified(self, ws):
        ...
        for l in range(0, ws.nrows):
            ...
            (last_account_class, last_account_group, root_account) = self.chart_title(ws.cell_value(l, 1),
                                                                                      ws.cell_value(l, 2),
                                                                                      last_account_class)

            if ws.cell_value(l, 1) in range(1000, 9999):

                last_account = self.create_account(ws.cell_value(l, 1), ws.cell_value(l, 2), None, chart.id,
                                                   owerwrite=True)
                ...
                if ws.cell_value(l, 4) in range(1000, 9999):
                    last_sub_account = self.create_account(ws.cell_value(l, 4), ws.cell_value(l, 5), last_account.id,
                                                           chart.id, owerwrite=True)

...
class wizard_multi_charts_accounts(models.TransientModel):
    ...
    def execute(self):
        ...
        account_values = {
            'UlagAvgHel': {'account_id': loner_till_tjansteman_7210.id,
                           'refund_account_id': loner_till_tjansteman_7210.id},
            'UlagVXLon': {'account_id': lon_vaxa_stod_tjansteman.id, 'refund_account_id': lon_vaxa_stod_tjansteman.id},
            'UlagAvgAldersp': {'account_id': loner_till_tjansteman_16_36.id,
                               'refund_account_id': loner_till_tjansteman_16_36.id},
            'UlagAlderspSkLon': {'account_id': loner_till_tjansteman_6_15.id,
                                 'refund_account_id': loner_till_tjansteman_6_15.id},
            'AvgHel': {'account_id': avrakning_lagstadgade_sociala_avgifter.id,
                       'refund_account_id': avrakning_lagstadgade_sociala_avgifter.id},
            'AvgVXLon': {'account_id': avrakning_sarskild_loneskatt.id,
                         'refund_account_id': avrakning_sarskild_loneskatt.id},
            'AvgAldersp': {'account_id': avrakning_lagstadgade_sociala_avgifter.id,
                           'refund_account_id': avrakning_lagstadgade_sociala_avgifter.id},
            'AvgAlderspSkLon': {'account_id': avrakning_lagstadgade_sociala_avgifter.id,
                                'refund_account_id': avrakning_lagstadgade_sociala_avgifter.id},
    ...
    def X_create_bank_journals_from_o2m(self, obj_wizard, company_id, acc_template_ref):
        ...
        for line in journal_data:
            ...
            while True:
                new_code = str(
                    ref_acc_bank.code[0:code_digits - len(str(current_num))].ljust(code_digits - len(str(current_num)),
                                                                                   '0')) + str(current_num)
...
class account_account_template(models.Model):
    ...
    def account2group(self, account_code):
        if ws.cell_value(l, 2) in range(1, 9) or ws.cell_value(l, 2) in [u'5–6']:  # kontoklass
            last_account_class = self.env['account.account.template'].create({
                'code': str(int(ws.cell_value(l, 2))) if isinstance(ws.cell_value(l, 2), float) else ws.cell_value(l,
                                                                                                                   2),
                'name': ws.cell_value(l, 3),

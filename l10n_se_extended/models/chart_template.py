...
class AccountChartTemplate(models.Model):
    ...
    def generate_journals(self, acc_template_ref, company, journals_dict=None):
        ...
        if not self == self.env.ref('l10n_se_extended.chart_template_k2_2017'):
            return res
    ...
    def _prepare_all_journals(self, acc_template_ref, company, journals_dict=None):
        ...
        if not self == self.env.ref('l10n_se_extended.chart_template_K2_2017'):
            return res
        account = acc_template_ref.get(self.env.ref('l10n_se_extended.K1_1010_2017').id)
        res.append({
            'type': 'general',
    ...
    def _prepare_transfer_account_for_direct_creation(self, name, company):
        ...
        if company.country_id.code == 'SE':
            xml_id = self.env.ref('l10n_se_extended.tag_a').id
